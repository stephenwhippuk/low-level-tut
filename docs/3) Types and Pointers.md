...
C and to a slightly lesser extent C++ have a very different philosophy. C will let you hang yourself, because it doesn't enforce things upon you in the same way, or at all. It simply gives you facilities to be able to do it more safely than trying it at machine level. C is statically ttped but its not strongly typed (or at least its easy to get round it).
...
This all said, the fact is you can if you obey the rules, right very safe C++ code, in a fraction of the time it takes to write a Rust alternative, and it'll be a whole lot more fun unless you really enjoy fighting with a compiler... you just cannot be as absolutely certain there aren't hidden gremlins in it.  
